...
class WGeolocation {
  ...
  factory WGeolocation() {
    if (_ddcSupported()) {
      return new WGeolocation._native();
    } else {
      return new WGeolocation._jsInterop();
    }
  }

  WGeolocation._jsInterop() {
    _ddcIsSafe = false;
  }

  WGeolocation._native() {
    _ddcIsSafe = true;
  }
  ...
  Future<Position> getCurrentPosition(
    ...
    if (_ddcIsSafe) {
      ...
      var jsGeoOptions = new _JsGeoOptions(
          ...
          maximumAge: timeout.inMilliseconds,
          enableHighAccuracy: enableHighAccuracy);
...
class _JsGeolocation {
  external static getCurrentPosition(_jsCallback success,
      [_jsCallback error, _JsGeoOptions params]);

  external static watchPosition(_jsCallback success,
      [_jsCallback error, _JsGeoOptions params]);
}
...
class _JsGeoOptions {
  ...
  external factory _JsGeoOptions(
      {int timeout, int maximumAge, bool enableHighAccuracy});
}
...
class Position {
  ...
  int get timestamp =>
      _isNativeInstance ? _asNative.timestamp : _asInterop.timestamp;
...
class Coordinates {
  ...
  double get latitude =>
      _isNativeInstance ? _asNative.latitude : _asInterop.latitude;

  double get longitude =>
      _isNativeInstance ? _asNative.longitude : _asInterop.longitude;

  double get altitude =>
      _isNativeInstance ? _asNative.altitude : _asInterop.altitude;

  double get altitudeAccuracy => _isNativeInstance
      ? _asNative.altitudeAccuracy
      : _asInterop.altitudeAccuracy;

  double get heading =>
      _isNativeInstance ? _asNative.heading : _asInterop.heading;
